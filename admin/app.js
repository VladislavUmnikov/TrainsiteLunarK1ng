...
/** Момент выезда/прибытия как реальный Date с учётом смещения поездки (по умолчанию MSK). Возвращает null, если дата/время не заполнены или некорректны. */
function getDepartureInstant(trip) {
  if (!trip.departureDate || !trip.departureTime) return null;
  const date = new Date(`${trip.departureDate}T${trip.departureTime}:00${trip.utcOffset || '+03:00'}`);
  return Number.isNaN(date.getTime()) ? null : date;
}

function getArrivalInstant(trip) {
  if (!trip.arrivalDate || !trip.arrivalTime) return null;
  const date = new Date(`${trip.arrivalDate}T${trip.arrivalTime}:00${trip.utcOffset || '+03:00'}`);
  return Number.isNaN(date.getTime()) ? null : date;
}

function getTravelDuration(trip) {
  const departure = getDepartureInstant(trip);
  const arrival = getArrivalInstant(trip);
...
function renderDashboard() {
  ...
  const upcoming = [...trips]
    .filter((trip) => {
      const departure = getDepartureInstant(trip);
      return departure && departure.getTime() >= now;
    })
    .sort((a, b) => getDepartureInstant(a) - getDepartureInstant(b))[0] ?? trips[0];
...
/** Код станции необязателен, но если он указан, он должен быть уникален в справочнике (без учёта регистра). excludeId позволяет игнорировать саму редактируемую станцию. */
function findStationCodeConflict(code, excludeId) {
  if (!code) return null;
  return state.stations.find((station) => station.id !== excludeId && (station.code || '').toUpperCase() === code) || null;
}
...
function bindEvents() {
  ...
  document.getElementById('admin-main').addEventListener('submit', (event) => {
    ...
    if (form.dataset.form === 'add-station') {
      const data = new FormData(form);
      const name = data.get('name').trim();
      if (!name) return;
      const code = data.get('code').trim().toUpperCase();
      const conflict = findStationCodeConflict(code);
      if (conflict) { showToast(`Код «${code}» уже используется станцией «${conflict.name}».`); return; }
      state.stations.push({ id: newId('station'), name, city: data.get('city').trim(), code, platforms: Number(data.get('platforms')) || 0 });
      saveData('stations', state.stations); renderStations(); showToast('Станция добавлена.');
    }
    if (form.dataset.form === 'edit-station') {
      const station = state.stations.find((item) => item.id === form.dataset.id);
      if (!station) return;
      const data = new FormData(form);
      const code = data.get('code').trim().toUpperCase();
      const conflict = findStationCodeConflict(code, station.id);
      if (conflict) { showToast(`Код «${code}» уже используется станцией «${conflict.name}».`); return; }
      Object.assign(station, { name: data.get('name').trim(), city: data.get('city').trim(), code, platforms: Number(data.get('platforms')) || 0 });
      saveData('stations', state.stations); showToast('Станция сохранена.');
    }
